...
For Ubuntu you just need to download the `.deb` file and do the following (the `mv` and `chmod` is just to deal with `apt` quickly giving up root priviliges and then having issues accessing files owned by you):
...
If you press the `<>` button, i.e. the "view code" button, seen in the title bar for such examples you'll find the HTML, TypeScript and CSS for the given example. If you've gone thru the Angular tutorial it's fairly obvious how this code should be added into an existing application. However these snippits miss out on some elements that need to be configured elsewhere in the application, i.e. the setup of your globabl CSS, your base `index.html` and your `@NgModule` section.

Note: sometimes VSCode or `ng serve` will claim that errors exist where they do not, this generally happens if you're using angular-cli to create new components etc. while VSCode or `ng serve` are running. So if you see an error that doesn't seem to make sense just try exiting and restarting VSCode and/or `ng serve` before you start investigating - this will often make the problem dissappear.

Basic Material application
--------------------------

So assuming you've already done the basic Angular tutorial we'll now build up a new app, and first pull in the Material [card](https://material.angular.io/components/card/examples) mentioned above and then tag of the [dialog](https://material.angular.io/components/dialog/examples) example.
...
Update [`styles.css`](angular-material-example/src/styles.css) to specift which Material theme you want to use. See the [theming](https://material.angular.io/guide/theming) guide for more details.

For the changes to `index.html` and `styles.css` see:
...
See `git show b0f3863` for this step.
...
If you're watching thing in the browser you'll see that everything breaks at this stage - you'll see more information in the console if you open the browser's developer tools.
...
But it's clear something's wrong with the styling of the buttons. As there's no issue in the console it's harder to know what to do here. If you look at the HTML for the buttons you'll see `mat-button` attributes and it turns out you need to include `MatButtonModule` to get the related CSS.

See `git show d10d19e` for this step.
...
Then update the code and the HTML for the console (pretty much exactly as in the Angular tutorial). This is pretty trivial - see `git show 9626472` for the changes involved.

### Adding the dialog example

Now we'll add the Material [dialog](https://material.angular.io/components/dialog/examples) example as this is a bit more involved - it involes two small forms and a small amount of code that uses the values entered in the forms and displays the dialog.
...
See `git show 43b48b5` for this step.

### Using the console

Let's get rid of the "You chose" text below the "Pick one" button and instead use the console that we added earlier. This is fairly trivial - see `git show fd0d5cf` for this step.
...
See `git show 7d4bfd8` for this step. We're missing a `pom.xml` for our super basic REST server, so let's add that.
...
See `git show 7155c7f` for this step.
...
See `git show 5b1dc94` for this step.
...
See `git show 5332039` for this step.
...
Note: CORS headers should not leak information - so if you specify an `Origin` header in your request that does not match the allowed origins configured on the backend then you'll get no CORS headers in the response. You only get CORS headers in the reponse in the positive situation where things match properly. This can make debugging a little difficult - if you don't get CORS headers back it's difficult to distinguish between the situation where there's an issue with your request and the `Origin` header specified or if CORS in misconfigured or not configured at all on the backend.
...
See `git show aef1374` for this step.

Important: if you jump to the implementation of `greetingUsingGET` you'll see there are variations that allow you to access more than just the unmarshalled body of the response, you can also get the full thing wrapped up as `HttpResponse`, with status code, headers etc. or even more exotic you can work with `HttpEvent` instances (that allow you to monitor the steps of the requests lifecycle in greater detail).
...
See `git show f60e459` for this step.
